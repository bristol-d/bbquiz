...
After `.filename`, which must be the first command, but before starting the first pool, you may optionally include any of the following commands:
  ...
  * `.question_preamble ARG` sets an argument that is included verbatim at the start of the rendered HTML of every question. This is useful if, for example, your blackboard installation contains a stylesheet that formats `<code>` blocks as non-monospace text and you want to add a style to revert to more sensible behaviour. The preamble is included at the start of every question to cater for the one-question-per-page display mode.
